...
df = pd.read_csv('WM-NEW-12-CURVED-FEET (2).csv')

velocity_x = df[df.columns[1]]

velocity_values = velocity_x[4301:5278].values.astype('float64')

velocity_y = df[df.columns[2]]

velocity_y_values = velocity_y[4301:5278].values.astype('float64')
...
force_values = force_x[4301:5278].values.astype('float64')

force_y = df[df.columns[7]]

force_y_values = force_y[4301:5278].values.astype('float64')
...
if __name__ == '__main__':
    ...
    phi_df.to_csv('phi_values_3_12-adv.csv')
