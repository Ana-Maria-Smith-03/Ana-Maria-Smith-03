...
df = pd.read_csv('WM_6-Advance-foot.csv')

velocity_x = df[df.columns[7]]

velocity_values = velocity_x[1098:1559].values.astype('float64')

velocity_y = df[df.columns[8]]

velocity_y_values = velocity_y[1098:1559].values.astype('float64')
...
force_values = force_x[1098:1559].values.astype('float64')

force_y = df[df.columns[2]]

force_y_values = force_y[1098:1559].values.astype('float64')
...
if __name__ == '__main__':
    ...
    phi_df.to_csv('phi_values_5_6-adv.csv')
